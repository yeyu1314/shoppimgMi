      ...
      <div class="container">
        <div class="cart-box">
          ...
          <ul class="cart-item-list">
            <li class="cart-item" v-for="(item, index) in listData" :key="index">
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      delProduct(item){ //删除购物车商品
        this.axios.delete(`/carts/${item.productId}`).then((res)=>{
          this.$message.success('删除成功');
          this.renderData(res);
        });
      },
      updateCart(item, type){ //加 1 或 减 1
      ...
      order(){//结算下单
        let isCheck = this.listData.every(item=>!item.productSelected); //返回true则表示商品都未勾选
        if(isCheck){
      ...
      renderData(res){ //公共赋值
        console.log(res)
        this.listData = res.cartProductVoList || []
        this.cartTotalQuantity = res.cartTotalQuantity
        this.allChecked = res.selectedAll
        this.cartTotalPrice = res.cartTotalPrice
        this.checkedNum = this.listData.filter(item => item.productSelected).length
      }
    }
  }
</script>
